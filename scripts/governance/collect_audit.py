                ...
                try:
                    with open(filepath, encoding="utf-8") as f:
                        ...
                        for match in matches:
                            token_usage.append(
                                {"file": os.path.relpath(filepath, root_dir), "token": match}
                            )
                except Exception:
                    pass
    return token_usage
...
def calculate_structure_digest(root_dir):
    ...
    for subdir, _dirs, files in os.walk(root_dir):
        if (
            "node_modules" in subdir
            or "venv" in subdir
            or ".git" in subdir
            or "__pycache__" in subdir
        ):
            continue
        for file in files:
...
def main():
    print("🔍 Starting Audit Collection...")
    ...
    evidence = {
        ...
        "validator_receipt": {
            ...
            "log": validator_result["stdout"][-500:]
